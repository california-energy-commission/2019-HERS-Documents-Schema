# ruby duplicate_elements.rb -p ../schema
...
parser = OptionParser.new do |opts|
  opts.banner = 'Usage: duplicate_elements.rb'
...
Dir.glob("#{project_path}/**/{CF1R,CF2R,CF3R,NRCV}/*.xsd").map do |schema|
  ...
  doc.xpath('/xsd:schema/xsd:complexType/xsd:sequence/xsd:element').map do |section|
    ...
    elements = section.xpath('.//xsd:element[not(@name = "Row")]/@name').map(&:to_s)
    duplicates = elements.select{|x| elements.count(x)>1}.uniq.join(', ')
    if elements.size != elements.uniq.size
      @error = 1
      puts "#{File.basename(schema)} has duplicate elements in section #{letter}\n"
      puts "Duplicate elements are #{duplicates}\n\n"
    end
  end
end
