# work in progress but it did pick
# up one error
# ruby choice.rb -p ../schema
...
Dir.glob("#{project_path}/**/{CF1R,CF2R,CF3R,NRCV}/*.xsd").map do |schema|
  ...
  doc.xpath('/xsd:schema/xsd:complexType/xsd:sequence/xsd:element').map do |section|
    ...
    section.xpath('.//xsd:choice').map do |choice|
      numbers = choice.xpath('xsd:element/@name').map{|x| x.to_s[1..2]}
      if numbers.uniq.size > 1
        @error = 1
        puts "#{File.basename(schema)} has incorrect xsd:choice sub element numbering (#{numbers.join(', ')}) in section #{letter}\n\n"
      end
    end
  end
end
